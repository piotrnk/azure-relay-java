...
public class HybridConnectionListener implements RelayTraceSource, AutoCloseable {
    ...
    public CompletableFuture<Void> closeAsync() {
        if (this.controlConnection.getClientWebsocket()!=null){
            try{
                this.controlConnection.getClientWebsocket().onClose(null, new CloseReason(CloseReason.CloseCodes.NORMAL_CLOSURE, "Client closing the socket normally"));
            }catch (Exception ex){
                ex.printStackTrace();
            }
        }
    ...
    static final class ControlConnection implements AutoCloseable {
        ...
        private volatile boolean closeCalled;
        private volatile ClientWebSocket clientWebsocket;
         ...
         */
        public synchronized CompletableFuture<Void> openAsync(Duration timeout) {
         ...
         *         is established between the control websocket and the cloud service
         */
        private synchronized CompletableFuture<Void> closeAsync(Duration duration) {
        ...
        private synchronized CompletableFuture<ClientWebSocket> connectAsync(Duration timeout) {
            try {
                ...
                clientWebsocket = new ClientWebSocket(this.listener.trackingContext, EXECUTOR);
                return delayTask.thenCompose(($void) -> {
                    return clientWebsocket.connectAsync(websocketUri, timeout, config)
                                    .thenApply(($void2) -> {
                                        this.onOnline();
...
/*                synchronized (this){
                    if (webSocket == null){
                        webSocket = new ClientWebSocket(this.listener.trackingContext, EXECUTOR);
                        return delayTask.thenCompose(($void) ->
                                webSocket.connectAsync(websocketUri, timeout, config)
                                         .thenApply(($void2) -> {
                                             this.onOnline();
                                             return webSocket;
                                         }));
        ...
        private synchronized CompletableFuture<Void> closeOrAbortWebSocketAsync(CompletableFuture<ClientWebSocket> connectTask, CloseReason reason) {
            ...
            return connectTask.thenCompose((webSocket) -> webSocket.closeAsync(reason))
                              .exceptionally((exception) -> {
                                  ...
                                  try {
                                      connectTask.get().closeAsync(reason);
        ...
        private CompletableFuture<Void> receivePumpAsync() {
            return receivePumpCoreAsync().handle((keepGoing, ex) -> {
                if (keepGoing) {
                    receivePumpAsync();
                    //openAsync(Duration.ofSeconds(5));
                } else {
                    if (ex != null) {
        ...
        private synchronized void onOnline() {
            synchronized (this.thisLock) {
                if (this.isOnline()) {
                    return;
                }
        ...
        private synchronized void onOffline(Throwable lastError) {
            if (lastError != null) {
        ...
        private synchronized boolean onDisconnect(Exception lastError) {
            synchronized (this.thisLock) {
                this.lastError = lastError;
